...
const ICON_LINK = `<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 0 1 1.242 7.244l-4.5 4.5a4.5 4.5 0 0 1-6.364-6.364l1.757-1.757m13.35-.622 1.757-1.757a4.5 4.5 0 0 0-6.364-6.364l-4.5 4.5a4.5 4.5 0 0 0 1.242 7.244"/></svg>`;
const ICON_CHECK = `<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5"/></svg>`;

const copyCardUrl = (btn, productId) => {
  navigator.clipboard.writeText(`https://pandcasa.com/cajita/${productId}`).then(() => {
    btn.innerHTML = ICON_CHECK;
    btn.classList.add('bg-brand-gold', '!border-brand-caramel/60');
    setTimeout(() => {
      btn.innerHTML = ICON_LINK;
      btn.classList.remove('bg-brand-gold', '!border-brand-caramel/60');
    }, 2000);
  }).catch(() => {});
};
  ...
  if (product.category === 'bocadillos_carousel') {
    return `
      <article class="card-reveal group relative overflow-hidden rounded-2xl border border-brand-caramel/20 bg-brand-cream shadow-soft hover:shadow-lift transition-shadow duration-300">
        <a href="/cajita/${product.id}" class="block h-[340px] w-full overflow-hidden relative">
        ...
        <button onclick="copyCardUrl(this,'${product.id}')" title="Copiar enlace"
          class="absolute bottom-3 left-3 w-10 h-10 rounded-full bg-brand-cream border border-brand-caramel/30 text-brand-caramel flex items-center justify-center shadow-md hover:scale-110 transition-transform duration-200">
          ${ICON_LINK}
        </button>
