...
const render = (product) => {
  ...
  el.innerHTML = `
    ...
    <!-- Detalle -->
    <div class="max-w-2xl mx-auto px-6 lg:px-10 py-12">
      ...
      <h1 class="mt-2 font-serif text-4xl md:text-5xl leading-tight text-brand-cocoa">${product.name}</h1>
      <p class="mt-3 text-2xl font-medium text-brand-caramel">${formatCRC(product.price)}</p>

      <div class="mt-3">${availability}</div>

      ${product.description ? `<p class="mt-6 text-brand-cocoa/70 leading-relaxed text-base">${product.description.replace(/\n/g, '<br>')}</p>` : ''}

      ${extras}

      <div class="mt-10 border-t border-brand-caramel/15 pt-8">
